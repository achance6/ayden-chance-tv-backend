...
public class FunctionRequestHandler extends MicronautRequestHandler<S3Event, Void> {
  ...
  @Override
  public Void execute(S3Event s3Event) {
	createMediaJob(s3Event);
	return null;
  }

  public void createMediaJob(S3Event s3Event) {

	String s3ObjectARN = parseS3EventARN(s3Event);
	CreateJobResponse createJobResponse;
	try (MediaConvertClient mediaConvertClient = MediaConvertClient.create()) {

	  LOGGER.debug("MediaConvert role ARN: {}", MEDIA_CONVERT_ROLE_ARN);
	  LOGGER.debug("MediaConvert input file ARN: {}", s3ObjectARN);
	  LOGGER.debug("MediaConvert output base path: {}", S3_OUTPUT_URI);
